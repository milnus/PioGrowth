format_calibrated_od_data <- function(raw_data, calibrated_data) {
  ...
  raw_data[["calibrated_data"]] <- raw_data[["filtered_data"]][["pioreactor_OD_data_wide"]]
  ...
  for (name in names(calibrated_data)) {
    ...
    hours_to_substitute <- raw_data[["calibrated_data"]]["hours"] %in% calibrated_data[[name]]["hours"]

    raw_data[["calibrated_data"]][hours_to_substitute, raw_data_col_oi] <- calibrated_data[[name]]["Calibrated_OD"]
  }
